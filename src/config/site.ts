export const SITE = {
  name: "نمایندگی تعمیرات ایسوس",
  domain: "asus-repairer.ir",
  url: "https://asus-repairer.ir",
  brandName: "Asus Repairer",
  locale: "fa-IR",
  language: "fa",
  phone: "+982191300348", 
  phoneDisplay: "021-91300348",
  whatsapp: "https://wa.me/09046972370",
  email: "info@asus-repairer.ir", 
  addressCentral: {
    street: "تهران ، خیابان مطهری ، ابتدای خیابان قائم مقام فراهانی جنوبی ، پلاک 158 ساختمان برتر سرویس​", 
    city: "Tehran",
    region: "Tehran",
    country: "IR",
    postalCode: "1586983711", 
  },
  addressWest: {
    street: "تهران، سعادت آباد، میدان کاج، کوچه دوازدهم علی اکبر، پلاک 30، مجتمع اداری کسری، طبقه اول واحد 5", // ✅ دقیق کن
    city: "Tehran",
    region: "Tehran",
    country: "IR",
    postalCode: "1586983711",
  },
  hours: [
    { day: "Saturday", opens: "09:00", closes: "19:00" },
    { day: "Sunday", opens: "09:00", closes: "19:00" },
    { day: "Monday", opens: "09:00", closes: "19:00" },
    { day: "Tuesday", opens: "09:00", closes: "19:00" },
    { day: "Wednesday", opens: "09:00", closes: "19:00" },
    { day: "Thursday", opens: "09:00", closes: "15:00" },
    { day: "Friday", opens: "11:00", closes: "14:00" },
  ],
  areasServed: [
    "تجریش",
"نیاوران",
"قلهک",
"دروس",
"زرگنده",
"جماران",
"ازگل",
"لویزان",
"مینی سیتی",
"سوهانک",
"حسین‌آباد",
"پاسداران",
"فرمانیه",
"کامرانیه",
"اقدسیه",
"چیذر",
"اختیاریه",
"قیطریه",
"دیباجی",
"دزاشیب",
"الهیه",
"زعفرانیه",
"ولنجک",
"اوین",
"درکه",
"در‌بند",
"سعادت‌آباد",
"شهرک غرب",
"فرحزاد",
"پونک",
"جنت‌آباد",
"شهران",
"کن",
"مرزداران",
"ستارخان",
"صادقیه",
"آریاشهر",
"اکباتان",
"تهرانسر",
"مهرآباد",
"یافت‌آباد",
"شهرک ولیعصر",
"نازی‌آباد",
"جوادیه",
"خزانه",
"شوش",
"مولوی",
"بازار",
"پامنار",
"عودلاجان",
"سنگلج",
"امیریه",
"میدان حر",
"میدان انقلاب",
"کارگر شمالی",
"کارگر جنوبی",
"فاطمی",
"یوسف‌آباد",
"امیرآباد",
"جلال‌آل‌احمد",
"گیشا",
"کوی نصر",
"ونک",
"ده ونک",
"آرارات",
"عباس‌آباد",
"سهروردی",
"بهار",
"نیلوفر",
"خواجه‌نصیر",
"نظام‌آباد",
"هفت‌تیر",
"سیدخندان",
"مجیدیه",
"دردشت",
"تهرانپارس غربی",
"تهرانپارس شرقی",
"حکیمیه",
"نارمک",
"لشگرک",
"شمیران‌نو",
"پیروزی",
"نیروی هوایی",
"پرستار",
"آهنگ",
"افسریه",
"مسعودیه",
"مشیریه",
"کیانشهر",
"خاوران",
"مینابی",
"زمزم",
"فلاح",
"شادآباد",
"عبدلال‌آباد",
"نعمت‌آباد",
"اسفندیاری",
"بریانک",
"هفت‌چنار",
"سلسبیل",
"نواب",
"حسن‌آباد",
"پاستور",
"ولیعصر",
"پارک لاله",
"جمهوری",
"فردوسی",
"لاله‌زار",
"بهارستان",
"میدان امام حسین",
"نامجو",
"امامزاده حسن",
"قصرالدشت",
"شهرک آزمایش",
"کوی بیمه",
"شهرک استقلال",
"شهرک آزادی",
"شهرک آپادانا",
"شهرک اکباتان فاز ۱",
"شهرک اکباتان فاز ۲",
"شهرک اکباتان فاز ۳",
"شهرک ژاندارمری",
"شهرک گلستان",
"چیتگر",
"دریاچه چیتگر",
"وردآورد",
"دهکده المپیک",
"شهرک راه‌آهن",
"زیبادشت",
"سازمان برنامه",
"کوی فراز",
"کوی دانشگاه",
"کوی فردوس",
"کوی مهران",
"شهرک محلاتی",
"کوی سوهانک",
"کوی نوبنیاد",
"کوی پاسداران",
"کوی ولیعصر",
"کوی نهم نیروی هوایی",
"قصر فیروزه",
"دولاب",
"سرآسیاب",
"شمشیری",
"یافت‌آباد شمالی",
"یافت‌آباد جنوبی",
"باغ‌فیض",
"آزادشهر",
"کوی ارم",
"چوب‌تراش",
"عبدل‌آباد",
"چهاردانگه",
"اسلام‌آباد",
"ولی‌آباد",
"خانی‌آباد نو",
"خانی‌آباد کهنه",
"بلوار ابوذر",
"افشار",
"پل چوبی",
"درب دوم",
"زرکش",
"قاسم‌آباد"
    
  ],
  social: {
    instagram: "https://www.instagram.com/bartar_repairer", 
  },
};

export const NAV = [
  { label: "صفحه اصلی", href: "/" },
  { label: "خدمات", href: "/services" },
  { label: "درباره ما", href: "/about" },
  { label: "تماس با ما", href: "/contact" },
];
